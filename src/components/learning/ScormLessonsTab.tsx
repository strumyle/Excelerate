...
export default function ScormLessonsTab() {
  ...
  const loadScormLessons = async () => {
    try {
      setLoading(true);
      console.log('Loading SCORM lessons...');
      ...
      const response = await fetch(`https://xrfiltyxdviefanplykg.supabase.co/functions/v1/list-scorm-lessons`, {
        method: "GET",
        headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(`list-scorm-lessons ${response.status}: ${body}`);
      }

      const data = await response.json();
      console.log('SCORM lessons response:', data);
      
      setLessons(data.lessons || []);
      console.log('SCORM lessons loaded:', data.lessons?.length || 0);
    } catch (error) {
    ...
    return (
      <div className="text-center py-12">
        ...
        <p className="text-muted-foreground">
          No SCORM lessons are available in your enrolled courses.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {lessons.map((lesson) => {
        const isCompleted = lesson.latest_attempt?.status === 'completed' || lesson.latest_attempt?.status === 'passed';
        const hasAttempt = !!lesson.latest_attempt;
          ...
          <Card key={lesson.id} className="hover:shadow-lg transition-shadow">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex gap-2">
                <Button
                  onClick={() => handlePlayScorm(lesson.scorm_package_id)}
                ...
                {isCompleted && (
                  <Button
                    onClick={() => navigate(`/courses/${lesson.modules.course_id}`)}
                    variant="outline"
                    size="icon"
                    title="View Course"
                  >
